...
bbox_data = []
for bbox_idx, (center, dimensions) in enumerate(Zones_fixes):
...
df_clusters.to_parquet(f'clusters_fixes.parquet', index=False)
df_bboxes.to_parquet(f'bounding_boxes_fixes.parquet', index=False)
...
while c<=5 :  # Changez à while True pour un traitement continu
    ...
    #------- Calcul des bounding box (optionnel) ---------------------------------
        ...
        # Écriture des DataFrames dans des fichiers Parquet
        df_clusters.to_parquet(f'clusters_{c}.parquet', index=False)
        df_bboxes.to_parquet(f'bounding_boxes_{c}.parquet', index=False)
