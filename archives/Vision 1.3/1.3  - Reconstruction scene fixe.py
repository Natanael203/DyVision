...
try:
    ...
    if point_clouds:
        ...
        downscaled_points = np.asarray(downscaled_pcd.points)
        filtered_points = filter_points_by_density(downscaled_points, radius, threshold)

        # Enregistrer le nuage de points combiné dans un fichier .ply
        output_filename = "combined_point_cloud.ply"
        o3d.io.write_point_cloud(output_filename, o3d.geometry.PointCloud(o3d.utility.Vector3dVector(filtered_points)))
        print(f"Nuage de points combiné et réduit enregistré dans {output_filename}")

        # Afficher le nuage de points combiné et réduit
        o3d.visualization.draw_geometries([o3d.geometry.PointCloud(o3d.utility.Vector3dVector(filtered_points))], window_name="Downscaled Combined Point Clouds")
